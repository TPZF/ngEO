...
/**
    ...
    @param coordinates
        {Array} The input coordinates
    @param checkDateLine
        {Boolean} When set to true, checks if feature crosses dateline.
            If positive, set <GeoBound> to cover the whole world
            Only used within intersectGeometry method to render properly crossing dateline bbox currently
 */
...
ImageRequest.prototype.abort = function()
{
    ...
    this.currentUrl = null;
	// this.image.src = ''; // Comment for now since it is a very bad idea
}
...
var TileRequest = function(tileManager)
{
	...
	var _handleLoadedImage = function() 
	{
		...
		if (!_imageLoaded)
		{
			...
			if ( _elevationLoaded )
			{
				var loadedImage = tileManager.pendingRequests.splice(tileManager.pendingRequests.indexOf(self), 1);
                if ( _imageRequest.currentUrl ) {
                    // console.log("handleLoadedImages: pushed to completed: " + self._id);
                    // console.log("handleLoadedImages: pushed to completed: " + _imageRequest.currentUrl);
                    // console.log("handleLoadedImages: pushed to completed(src): " + _imageRequest.image.src);
    ...
    {
        var erronousRequest = tileManager.pendingRequests.splice(tileManager.pendingRequests.indexOf(self), 1);
        if ( _imageRequest.currentUrl )
        {
            // console.log("handleErrorImage: pushed: " + self._id);
            // console.log("handleErrorImage: pushed: " + _imageRequest.currentUrl);
            // console.log("handleErrorImage: pushed to completed(src): " + _imageRequest.image.src);
	...
	{
        var abortedRequest = tileManager.pendingRequests.splice(tileManager.pendingRequests.indexOf(self), 1);
        if ( _imageRequest.currentUrl )
        {
            // console.log("handleAbort: pushed: " + self._id);
            // console.log("handleAbort: pushed: " + _imageRequest.currentUrl);
            // console.log("handleAbort: pushed to completed(src): " + _imageRequest.image.src);
	...
	var _handleLoadedElevation = function() 
	{
		...
		if ( _imageLoaded )
		{
			var loadedRequest = tileManager.pendingRequests.splice(tileManager.pendingRequests.indexOf(self), 1);
            if ( _imageRequest.currentUrl )
            {
                // console.log("_handleLoadedElevation: pushed to completed: " + self._id);
                // console.log("_handleLoadedElevation: pushed to completed: " + _imageRequest.currentUrl);
                // console.log("_handleLoadedElevation: pushed to completed(src): " + _imageRequest.image.src);
    			tileManager.completedRequests.push(self);
    			tileManager.renderContext.requestFrame();
            }
		}
	...
	var _handleErrorElevation = function() 
	{
		...
		if ( _imageLoaded )
		{
            ...
            // console.log("_handleErrorElevation: pushed to completed: " + self._id);
            // console.log("_handleErrorElevation: pushed to completed: " + _imageRequest.currentUrl);
            // console.log("_handleErrorElevation: pushed to completed(src): " + _imageRequest.image.src);
			tileManager.completedRequests.push(self);
			tileManager.renderContext.requestFrame();
		}
	}
...
//        this.availableRequests[i]._id = i;
	}
 ...
 */
 TileManager.prototype.generateReceivedTiles = function()
 {
	while ( this.completedRequests.length > 0 )
	{
		...
		/*
        console.log("GenerateReceivedTiles: pushed: " + tileRequest._id);
        if ( tileRequest._imageRequest )
            console.log("GenerateReceivedTiles: pushed: " + tileRequest._imageRequest.currentUrl);
		*/
 ...
 */
 TileManager.prototype.launchRequests = function()
 {
	...
	for ( var i = 0; i < trl; i++ )
	{
		...
		{
            // console.log("availableRequests.length",this.availableRequests.length);
...
Globe.prototype.get3DFromPixel = function(x,y)
{	
	var ray = Ray.createFromPixel(this.renderContext, x, y);
	var intersection;
	if ( this.coordinateSystem.isFlat )
	...
	if ( intersection >= 0 )
	{
		return ray.computePoint(intersection);
	}
	else
	{
		return null;
...
{	
	...
	var levelZeroIndex = Math.floor( x * this.level0NumTilesX );
...
{	
	...
	var y = Math.max(0, 0.5 - Math.log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI)); // Hack: Clamp to 0 when lat > 85.05
...
/** @name WMSLayer
	...
	@param options Configuration properties for the WMSLayer. See {@link RasterLayer} for base properties :
		<ul>
			...
			<li>projection : the spatial system reference to use, default is EPSG:4326 (WMS parameter)</li>
			<li>format : the file format to request, default is image/jpeg (WMS parameter)</li>
			<li>tilePixelSize : the pixel size to use, default is 256(WMS parameter)</li>
		</ul>
 */
var WMSLayer = function( options )
{
	...
	switch (options.projection) {
		case "EPSG:3857":
			this.tiling = new MercatorTiling( 2 );
			break;
		case "EPSG:4326":
		default:
...
/** @export
    ...
    A layer to display WMTS (Web Map Tile Service) data.
    @augments RasterLayer
    @param options Configuration properties for the WMTSLayer. See {@link RasterLayer} for base properties :
        <ul>
            <li>baseUrl : the base Url to access the WMTS server</li>
            <li>layers : the list of layers to request (WMTS parameter)</li>
            <li>projection : the spatial system reference to use, default is EPSG:4326 (WMTS parameter)</li>
            <li>format : the file format to request, default is image/jpeg (WMTS parameter)</li>
        </ul>
 */
var WMTSLayer = function( options )
{
	...
	this.startLevel = options.hasOwnProperty('startLevel') ? options['startLevel'] : 1;
